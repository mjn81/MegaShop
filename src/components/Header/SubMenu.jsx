...
const SubMenu = ({item, isActive}) => {
    ...
    return ( 
        <li className='my-2 border-y-2 border-white hover:bg-gray-50 hover:border-gray-100' onMouseEnter={mouseEnterHandler} onMouseLeave={mouseLeaveHandler}>
            <Link to={item.path} className='block py-3 px-5 hover:text-primary'>
                {item.title}
            </Link>
            {item.subMenu && <div id="menu-hover" className={`${isActive ? 'custom-shadow-right' : ''} top-0 left-full h-full grid grid-cols-4 gap-x-28 gap-y-6 py-4 px-2 rounded-br-lg absolute w-max border-l-2 bg-white ${isHovered ? 'active' : 'deactive'}`}>
